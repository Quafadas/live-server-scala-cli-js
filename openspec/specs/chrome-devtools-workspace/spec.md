# chrome-devtools-workspace Specification

## Purpose

Enables Chrome DevTools M-135+ "Automatic Workspace Folders" by serving a well-known JSON descriptor during local development. When configured, the dev server responds to `GET /.well-known/appspecific/com.chrome.devtools.json` so that DevTools can automatically mount the project source folder without any manual configuration.
